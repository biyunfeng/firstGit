    ...
    bool& edge_to_keyframe) ///Output:contains the best-yet of the pairwise motion estimates for the current node
{
    ...
    bool keep_anyway = (ps->get<bool>("keep_all_nodes") || (((int)new_node->feature_locations_3d_.size() > ps->get<int>("min_matches")) && ps->get<bool>("keep_good_nodes")));
...
double GraphManager::optimizeGraphImpl(double break_criterion)
{
    ...
    } else //Got the lock
    {
        ...
        ROS_WARN_STREAM_NAMED("eval", "G2O Statistics: " << std::setprecision(15) << camera_vertices.size() << " cameras, " << optimizer_->edges().size() << " edges. " << chi2 << " ; chi2 "
